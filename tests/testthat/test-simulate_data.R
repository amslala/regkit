...
test_that("Year of birth", {
  ...
  filtered_df <- regkit::filter_diag_data(test$datasets$diag_df, pattern_codes = c("F45", "F84"), code_col = "code", id_col = "id", log_path = l_path)

  curated_df <- regkit::curate_diag_data(filtered_df, min_diag = 1, first_diag = TRUE, code_col = "code", date_col = "diag_year", log_path = l_path)

  invar_test <- test$datasets$invar_df

  filtered_df_2 <- regkit::filter_admin_data(invar_test, data_type = "t_invariant", filter_param = list("innvandringsgrunn" = c("ARB", "NRD", "UKJ")), id_col = "id", rm_na = FALSE, log_path = l_path)

  linked_df <- regkit::link_diag_admin(curated_df, data_admin_inv = filtered_df_2, id_col = "id", log_path = l_path)
