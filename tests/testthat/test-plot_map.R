...
test_that("Correct type of plot in layers ggplot object", {
  ...
  pop_kommuner_2024 <- regkit::get_population_ssb(regions = "kommuner", years = 2024, ages = c(0:18), aggregate_age = TRUE, by_sex = FALSE)

  pop_kommuner_2024 <- regkit::harmonize_municipality_codes(data = pop_kommuner_2024, municipality_col = "region_code") |>
...
test_that("Correctly joins kommune or fylke geographical data", {
  ...
  pop_fylker_2024 <- regkit::get_population_ssb(regions = "fylker", years = 2024, ages = c(0:18), aggregate_age = TRUE, by_sex = FALSE) |>
    dplyr::filter(age == "Total")
