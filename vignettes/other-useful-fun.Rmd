...
This vignette describes some additional function in `regkit` that can be useful when working with registry data in Norway. It is important to note that the `get_population_ssb()` function requires an internet connection and therefore only works outside of TSD (Services for sensitive data). 
