...
To enable users to work with larger-than-memory data within `regtools`, the functions `read_diag_data()` `read_demo_data()` `filter_diag_data()` `filter_demo_data()` support parquet files. This is particularly helpful to speed-up the filtering steps in very large data sets. In the next example, it is shown how to write/read parquet datasets and use them as part of the `regtools` analytical pipeline. 
